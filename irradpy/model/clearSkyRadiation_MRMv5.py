...
class ClearSkyMRMv5:
    ...
    def MRMv5(self, sza, wv, albedo, ozone, ang_beta):
        ...
        am = 1 / (cos(sza) + 0.50572 * power((pi / 2 - sza) / pi * 180 + 6.07995, -1.6364))
        ama = am * self.press / 1013.25

        # atmospheric gases transmittance function
        Tco2 = 1 - (0.07210 * ama * 350 / (power(1 + 377.890 * ama * 350, 0.5855) + 3.1709 * ama * 350))
        Tco = 1 - (0.0062 * ama * 0.075 / (power(1 + 243.670 * ama * 0.075, 0.4246) + 1.7222 * ama * 0.075))
        Tn2o = 1 - (0.0326 * ama * 0.28 / (power(1 + 107.413 * ama * 0.28, 0.5501) + 0.9093 * ama * 0.28))
        Tch4 = 1 - (0.0192 * ama * 1.6 / (power(1 + 166.095 * ama * 1.6, 0.4221) + 0.7186 * ama * 1.6))
        To2 = 1 - (0.0003 * ama * 2.095e5 / (power(1 + 476.934 * ama * 2.095e5, 0.4892) + 0.1261 * ama * 2.095e5))
        ...
        Tw = 1 - (3.0140 * am * lw / (power(1 + 119.300 * am * lw, 0.6440) + 5.8140 * am * lw))

        # the transmittance function for ozone  #because we have ozone, use eq.14.27,otherwise eq.14.9
        To = 1 - (0.2554 * am * ozone / (power(1 + 6107.260 * am * ozone, 0.2040) + 0.4710 * am * ozone))

        # the transmittance function for Rayleigh scattering
        Tr = exp(-0.1128 * power(ama, 0.8346) * (0.9341 - power(ama, 0.9868) + 0.9391 * ama))
        ...
        Taa = 1 - 0.1 * (1 - am + power(am, 1.06)) * (1 - Ta)
        ...
        Ta166 = exp(-1.66 * ang_beta * power(0.6777 + 0.1464 * (1.66 * ang_beta) - 0.00626 * power(1.66 * ang_beta, 2), -1.3))
        ...
        return [EbnMRM5, EdhMRM5, EghMRM5]

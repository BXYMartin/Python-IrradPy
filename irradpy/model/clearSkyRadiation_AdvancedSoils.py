...
class ClearSkyAdvancedSoils:
    ...
    def AdvancedSoils(self, sza, wv, aod550):
        ...
        b = cbb1 * power(pw, cbb2) * power(cbb3, aod)
        g = ca1 + ca2 * pwln + ca3 * aodln + ca4 * pwln * pwln + ca5 * aodln * aodln + ca6 * pwln * aodln
        ETcor = ET * En0

        # direct normal radiation
        Ebnsolis18 = ETcor * exp(-taub / (power(cos(sza), b)))

        # global horizontal irradiance
        Eghsolis18 = ETcor * exp(-taug / (power(cos(sza), g))) * (cos(sza))
        ...
        return [Ebnsolis18, Edhsolis18, Eghsolis18]

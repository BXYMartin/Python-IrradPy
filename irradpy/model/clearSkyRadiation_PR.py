...
class ClearSkyPR:
    ...
    def PR(self, sza, wv, albedo, ozone, ang_beta):
        ...
        amky = 1 / (cos(sza) + 0.50572 * power((pi / 2 - sza) / pi * 180 + 6.07995, -1.6364))
        amkyp = amky * self.press / 1013.25

        # transmittances from Psiloglou2000
        # Badescu added the presure correction where needed for cinsistency
        Trr = exp(-0.1128 * power(amkyp, 0.8346) * (0.9341 - power(amkyp, 0.9868) + 0.9391 * amkyp))
        ako3 = amky * ozone
        Tro = 1 - 0.2554 * ako3 / (power(1 + 6107.26 * ako3, 0.204) + 0.471 * ako3)
        akco2 = amkyp * 330
        TrCO2 = 1 - 0.0721 * akco2 / (power(1 + 377.89 * akco2, 0.5855) + 3.1709 * akco2)
        akco = amkyp * 0.075
        TrCO = 1 - 0.0062 * akco / (power(1 + 243.67 * akco, 0.4246) + 1.7222 * akco)
        akch4 = amkyp * 1.6
        TrCH4 = 1 - 0.0192 * akch4 / (power(1 + 166.095 * akch4, 0.4221) + 0.7186 * akch4)
        akn2o = amkyp * 0.28
        TrN2O = 1 - 0.0326 * akn2o / (power(1 + 107.413 * akn2o, 0.5501) + 0.9093 * akn2o)
        ako2 = amkyp * 2.095e5
        TrO2 = 1 - 0.0003 * ako2 / (power(1 + 476.934 * ako2, 0.4892) + 0.1261 * ako2)
        Trmg = TrCO2 * TrCO * TrCH4 * TrN2O * TrO2
        akw = amky * wv
        Trw = 1 - 3.014 * akw / (power(1 + 119.3 * akw, 0.644) + 5.814 * akw)
        ...
        Traa = 1 - 1.405e-3 * amky - 9.013e-5 * power(amky, 2) + 2.2e-6 * power(amky, 3)
        Tras = Tra / Traa
        ros = 0.08503
